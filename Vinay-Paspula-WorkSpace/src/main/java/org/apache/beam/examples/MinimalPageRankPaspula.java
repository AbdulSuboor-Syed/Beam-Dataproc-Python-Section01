 ...
 * file service.
 */
public class MinimalPageRankPaspula {
  ...
  public static void main(String[] args) {
    ...
    // We want to read from a file - just one - we need the file name - assign to a
    // variable.

    String folder = "web04";
    // String file="go.md";
    PCollection<KV<String, String>> PCollection_KV_1 = paspulaMapper1(p, "go.md", folder);
    PCollection<KV<String, String>> PCollection_KV_2 = paspulaMapper1(p, "python.md", folder);
    PCollection<KV<String, String>> PCollection_KV_3 = paspulaMapper1(p, "java.md", folder);
    PCollection<KV<String, String>> PCollection_KV_4 = paspulaMapper1(p, "README.md", folder);

    PCollectionList<KV<String, String>> PCollection_List = PCollectionList.of(PCollection_KV_1).and(PCollection_KV_2)
        .and(PCollection_KV_3).and(PCollection_KV_4);
    PCollection<KV<String, String>> l = PCollection_List.apply(Flatten.<KV<String, String>>pCollections());
    PCollection<String> PCollection_String = l
        .apply(MapElements.into(TypeDescriptors.strings()).via((Out) -> Out.toString()));
    PCollection_String.apply(TextIO.write().to("paspulaout"));
    p.run().waitUntilFinish();
    // paspulaMapper1(p);
    // PCollection_Links.apply(TextIO.write().to("paspulaout"));
  ...
  private static PCollection<KV<String, String>> paspulaMapper1(Pipeline p, String dataFile, String dataFolder) {
    String dataPath = dataFolder + "/" + dataFile;
    PCollection<String> pcolInputLines = p.apply(TextIO.read().from(dataPath));
    PCollection<String> pcolLinkLines = pcolInputLines.apply(Filter.by((String line) -> line.startsWith("[")));
    PCollection<String> PCollection_Links = pcolLinkLines.apply(MapElements
        .into(TypeDescriptors.strings())
        .via(
            (String linkline) -> linkline.substring(linkline.indexOf("(") + 1, linkline.length() - 1)));

    // from README.md to Key Value Pairs
    PCollection<KV<String, String>> PC_Key_Value = PCollection_Links.apply(MapElements
